            ...
            if L_1.startswith('[{"lat":'):
                ...
                for L_3 in L_2:
                    if L_3.startswith('[{"lat":') and L_3.endswith('}]'):
                        L_str = L_3.replace('[', '').replace('{"lat":', '').replace(',"lon":', ',') \
                            .replace('},', '; ').replace('}]}]', '')
                        Lturn_1 = L_str.split('; ')
                        my_Linestring = ''
                        ...
                        print(URL[:5] + ": LINESTRING (" + my_Linestring[:-2] + ")")
                ...
                if M_step2.endswith(',"strokeWeight":"2"'):  # 去除中段非坐標訊息
                    ...
                    for M_step4_end in M_step3_end:
                        if M_step4_end.startswith('[{"lat":'):
                            ...
                            # print(Mend_list)
        print(URL[:5] + ": MULTILINESTRING (" + Mmid_final + Mend_list)  # 加入最後一段Linestring

    else:  # 不符Global Energy Monitor原始碼規則
        print(URL[:5] + ": Not in the rule. Please check!")
