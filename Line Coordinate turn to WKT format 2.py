...
row = 1
while row < 13:  # row < 任意數字，但要小於資料總筆數
    for line in cellRange:  # 列出坐標
        origional = line.value
        # print(origional)

        if 'Capacity expansion only' in origional:  # 原始資料為Capacity expansion only
            print("Capacity expansion only")
            sht.cell(row, 3).value = "--"  # 寫入excel儲存格
            row += 1  # excel換行

        elif 'Unavailable' in origional:  # 原始資料為Unavailable
            print("Unavailable")
            sht.cell(row, 3).value = "--"  # 寫入excel儲存格
            row += 1  # excel換行

        elif ';' in origional:  # 篩選出屬於Multilinestring的資料
